      ...
      while (true) {
        ...
        html: `
          ...
          <div class="staff-form-grid">
            ...
            <div>
              <label style="display: block; margin-bottom: 0.5rem; font-weight: 600; color: #333;">Present Residential State</label>
              <select id="swalStaffPresentResidentialState" class="swal2-select" style="width: 100%; padding: 0.75rem; border: 1px solid #ddd; border-radius: 4px; font-size: 1rem;">
                <option value="">-- Select State --</option>
                ${(window.NIGERIA_STATES || []).map(st => `<option value="${(st || '').replace(/"/g, '&quot;')}" ${(step2InitialData.presentResidentialState || '') === st ? 'selected' : ''}>${String(st || '').replace(/</g, '&lt;')}</option>`).join('')}
              </select>
            </div>
            <div>
              <label style="display: block; margin-bottom: 0.5rem; font-weight: 600; color: #333;">Present Residential City</label>
              <input id="swalStaffPresentResidentialCity" class="swal2-input" placeholder="City" value="${(step2InitialData.presentResidentialCity || '').replace(/"/g, '&quot;')}" style="width: 100%; padding: 0.75rem; border: 1px solid #ddd; border-radius: 4px; font-size: 1rem;">
            </div>
            <div style="grid-column: 1 / -1;">
              <label style="display: block; margin-bottom: 0.5rem; font-weight: 600; color: #333;">Present Residential Address</label>
        ...
        preConfirm: async () => {
          ...
          const permanentHomeAddress = document.getElementById('swalStaffPermanentAddress')?.value.trim() || '';
          const presentResidentialState = document.getElementById('swalStaffPresentResidentialState')?.value.trim() || '';
          const presentResidentialCity = document.getElementById('swalStaffPresentResidentialCity')?.value.trim() || '';
          const presentResidentialAddress = document.getElementById('swalStaffPresentResidentialAddress')?.value.trim() || '';
          const stateSecretariatAppointmentLetter = document.getElementById('swalStaffStateSecretariat')?.value.trim() || '';
          ...
          return { step2Data: { maritalStatus, spouseName, nextOfKin, permanentHomeAddress, presentResidentialState, presentResidentialCity, presentResidentialAddress, stateSecretariatAppointmentLetter }, profilePictureData };
        }
        });
        ...
        try {
          ...
          const res = await Api.call('createStaff', {
            ...
            staff: {
              ...
              presentResidentialState: step2Data.presentResidentialState || '',
              presentResidentialCity: step2Data.presentResidentialCity || '',
  ...
  async function downloadStaffRecordsAsCsv() {
    ...
    try {
      ...
      const columns = [
        ...
        'confirmationDate', 'qualification', 'maritalStatus', 'spouseName', 'nextOfKin', 'permanentHomeAddress',
        'presentResidentialState', 'presentResidentialCity', 'presentResidentialAddress', 'stateSecretariatAppointmentLetter',
        'formationId', 'subUnitId'
      ];
  ...
  async function viewStaffProfile(employeeId, formationId) {
    ...
    try {
      ...
      try {
        ...
        const docBody = { key: adminKey, employeeId: employeeId };
        if (staff.systemRecordID) docBody.systemRecordID = staff.systemRecordID;
        if (formationId) docBody.formationId = formationId;
        const docRes = await Api.call('listStaffDocuments', docBody);
        UI.closeLoading();
        if (docRes && docRes.success && docRes.data && docRes.data.documents) {
          documents = docRes.data.documents;
        ...
        didOpen: () => {
          ...
          if (viewFullBtn) {
            viewFullBtn.addEventListener('click', () => {
              Swal.close();
              showFullStaffProfile(employeeId, staff.formationId || '', staff.systemRecordID || '');
            });
          }
        }
      });
...
<body>
  ...
  async function editStaff(employeeId, formationId, systemRecordID) {
    if (!adminKey) return;
      ...
      const result = await Swal.fire({
        ...
        html: `
          ...
          <div class="staff-form-grid">
            ...
            <div>
              <label style="display: block; margin-bottom: 0.5rem; font-weight: 600; color: #333;">Present Residential State</label>
              <select id="swalEditPresentResidentialState" class="swal2-select" style="width: 100%; padding: 0.75rem; border: 1px solid #ddd; border-radius: 4px; font-size: 1rem;">
                <option value="">-- Select State --</option>
                ${(window.NIGERIA_STATES || []).map(st => `<option value="${(st || '').replace(/"/g, '&quot;')}" ${(staff.presentResidentialState || '') === st ? 'selected' : ''}>${String(st || '').replace(/</g, '&lt;')}</option>`).join('')}
              </select>
            </div>
            <div>
              <label style="display: block; margin-bottom: 0.5rem; font-weight: 600; color: #333;">Present Residential City</label>
              <input id="swalEditPresentResidentialCity" class="swal2-input" placeholder="City" value="${staff.presentResidentialCity || ''}" style="width: 100%; padding: 0.75rem; border: 1px solid #ddd; border-radius: 4px; font-size: 1rem;">
            </div>
            <div style="grid-column: 1 / -1;">
              <label style="display: block; margin-bottom: 0.5rem; font-weight: 600; color: #333;">Present Residential Address</label>
            ...
            if (actionsDiv) {
              ...
              if (!updateBtn) {
                ...
                updateBtn.addEventListener('click', async () => {
                  ...
                  const presentResidentialStateEl = document.getElementById('swalEditPresentResidentialState');
                  const presentResidentialCityEl = document.getElementById('swalEditPresentResidentialCity');
                  const presentResidentialAddressEl = document.getElementById('swalEditPresentResidentialAddress');
                  const stateSecretariatEl = document.getElementById('swalEditStateSecretariat');
                  ...
                  const presentResidentialState = presentResidentialStateEl ? presentResidentialStateEl.value.trim() : '';
                  const presentResidentialCity = presentResidentialCityEl ? presentResidentialCityEl.value.trim() : '';
                  const presentResidentialAddress = presentResidentialAddressEl ? presentResidentialAddressEl.value.trim() : '';
                  const stateSecretariatAppointmentLetter = stateSecretariatEl ? stateSecretariatEl.value.trim() : '';
                  ...
                  try {
                    ...
                    const updatePayload = {
                      ...
                      staff: {
                        ...
                        presentResidentialState: presentResidentialState || '',
                        presentResidentialCity: presentResidentialCity || '',
                        presentResidentialAddress: presentResidentialAddress || '',
                        stateSecretariatAppointmentLetter: stateSecretariatAppointmentLetter || '',
                        status
                      }
                    };
